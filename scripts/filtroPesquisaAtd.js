...
searchInput.addEventListener('input', function () {
   ...
   var rows = document.querySelectorAll('table tr:not(:first-child)');
    ...
    /// Loop através das linhas da tabela e verifica se o nome ou protocolo corresponde ao termo de pesquisa
    rows.forEach(function (row) {
        var nome = row.cells[1].textContent.toLowerCase(); // Obtém o nome da célula
       var protocolo = row.cells[0].textContent.toLowerCase(); // Obtém o protocolo da célula

        /// Verifica se o nome ou protocolo contém o termo de pesquisa
        if (nome.includes(searchTerm) || protocolo.includes(searchTerm)) {
